...
//Checks if a tile intersects a line made up of x1y1 x2y2
function checkLOS(x1, y1, x2, y2){
    ...
    for(let t of tileManager.tiles){
        let x = t.x;
        let y = t.y;
        //Checks if the tile is within the bounds of the rectangle drawn by the line
        if( ((x - x1) * (x - x2) <= 0) && ((y - y1) * (y - y2) <= 0) ){
            //slope
            let a = (y1-y2)/(x1-x2);
            //always -1 because u solve with coeff of 1
            let b = -1;
            //technically b
            let c = y1 - a*x1;
            let dist = (Math.abs(a * x + b * y + c)) / Math.sqrt(a * a + b * b);
            //Use half width of tile as the radius here
            if(dist < Math.sqrt(t.width*t.width*2)/2)
                return false;
        }
        else if(  ( Math.abs((xMax + xMin)/2  - x)  <= t.width/2) &&  ( Math.abs((yMax + yMin)/2 - y)  <= t.width/2) ){
            //slope
            let a = (y1-y2)/(x1-x2);
            //always -1 because u solve with coeff of 1
            let b = -1;
