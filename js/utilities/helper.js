...
//Loads a sprite sheet with multiple rows
function loadSpriteSheetRows(source, rows = 0, width = 0, height = 0, frames = 0, totalFrames = 0) {  
    let baseTexture = new PIXI.BaseTexture.from(source);
    baseTexture.scaleMode = PIXI.SCALE_MODES.NEAREST;    
    let sprites = [];
    for(let r = 0; r < rows; r++){
        for (let i = 0; i < frames; i++) {
            if(sprites.length >= totalFrames)
                break;
            let sprite = new PIXI.Texture(baseTexture, new PIXI.Rectangle(i * width, r*height, width, height));
            sprites.push(sprite);
        }  
    }
    return sprites;
...
function loadSound(source, volume, loop = false){
    let sound = new Howl({
